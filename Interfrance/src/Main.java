    ...
    public static void loadAccounts() throws IOException {
        Files.lines(paths[0])
                .forEach(line -> {
                    ...
                    if (data[0].equals(AccountType.CHECKING.toString())) {
                        checkingService.createAccount(new Checking(data[1], new BigDecimal(data[2])));
                    } else {
                        creditService.createAccount(new Credit(data[1], new BigDecimal(data[2])));
                    }
                });
    }
        public static void applyTransactions() throws IOException{
            Files.lines(paths[1])
                    .forEach(line ->{
                        ...
                        Boolean isChecking = word[0].equals(AccountType.CHECKING.toString());
                        AccountService accountService = isChecking? checkingService: creditService;
                        Transaction transaction = Transaction.valueOf(word[2]);
                        if(transaction.equals(Transaction.DEPOSIT)){
                            accountService.deposit(word[1],new BigDecimal(word[3]) );
                        }else{
                            accountService.withdraw(word[1],new BigDecimal(word[3]));
                        }
                    });
